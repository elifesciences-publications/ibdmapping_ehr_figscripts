...
x <- read.table("PR_3Copies.linear.recessive.5PCs.assoc.linear.FINAL", header=F)
...
manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
    ...
    if (genomewideline) abline(h=genomewideline, col="firebrick",lty=2,lwd=2)
    
    # Highlight snps from a character vector
    if (!is.null(highlight)) {
        if (any(!(highlight %in% d$SNP))) warning("You're trying to highlight SNPs that don't exist in your results.")
        d.highlight=d[which(d$SNP %in% highlight), ]
        with(d.highlight, points(pos, logp, col="firebrick", pch=20, ...)) 
    }
    ...
    if (!is.null(annotatePval)) {
        ...
        if (annotateTop == FALSE) {
            ...
            textxy(topSNPs$pos, -log10(topSNPs$P), offset = 0.5, labs = topSNPs$SNP, cex = 1, ...)
        }
    }  
    par(xpd = FALSE)}
tiff("FIG2A.tiff",height=600,width=1000)
...
manhattan(x,col=c("skyblue1","skyblue4"),suggestiveline=FALSE,cex=4,genomewideline=-log10((0.05/480)),cex.axis=1.5,cex.lab=1.5,ylab="",ylim=c(-0.5,14))
title(ylab="-log10(P)", line=2.5, cex.lab=1.5)
box(lwd=2) 
dev.off()
